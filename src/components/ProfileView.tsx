...
export const ProfileView: React.FC<ProfileViewProps> = ({ gameState, userProfile }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      {/* Main Profile Card */}
      <div className="mq-glass p-8 text-center space-y-6 relative overflow-hidden border-slate-800">
        ...
        <div className="space-y-1">
          <h2 className="text-3xl font-extrabold text-white">Chevalier des Mathématiques</h2>
          <p className="text-xs text-slate-400 font-medium">Parcours de Réussite de la 3ème à la Terminale</p>
        </div>

        {/* Quick Stats Grid */}
        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-slate-800/80">
          <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-800">
            <span className="block text-2xl font-black text-amber-400">🪙 {gameState.gold}</span>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Or accumulé</span>
          </div>

          <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-800">
            <span className="block text-2xl font-black text-purple-400">Niv. {gameState.level}</span>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Niveau Atteint</span>
          </div>

          <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-800">
            <span className="block text-2xl font-black text-emerald-400">{userProfile.xp} XP</span>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">XP Totale</span>
          </div>
        </div>
      </div>
      ...
      <div className="mq-glass p-6 space-y-3 border-slate-800">
        <div className="flex items-center justify-between text-xs font-bold">
          <span className="text-slate-300 uppercase tracking-wider">Indicateur de Préparation Seconde & Lycée</span>
          <span className="text-amber-400">{readinessPct}% Prêt</span>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {trophies.map((t) => (
            <div
              key={t.id}
              className={`mq-glass p-4 rounded-xl border flex items-center gap-4 transition-all ${
                t.isUnlocked
